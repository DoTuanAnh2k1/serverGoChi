...
Create ──► Enabled ──► Login ──┬──► Change password ──► same Enabled state
             ...
             Disabled ◄── Admin clicks "Disable"
                ...
                │
                ▼
             PURGED — hard-delete row + mappings + history, irreversible
```
...
| **Disable** | `POST /aa/authenticate/user/delete` | Soft-delete: set `is_enable=false`. SuperAdmin không xoá được. Row vẫn còn trong DB, có thể re-enable. |
| **Purge (hard-delete)** | `POST /aa/authenticate/user/purge` | Xoá hẳn row `tbl_account` + mọi mapping (user↔group, user↔NE) + password history. **Không khôi phục được.** SuperAdmin refuse (403). Frontend yêu cầu gõ lại username để xác nhận; CLI `purge user <name>` có y/N confirm. `cli_operation_history` giữ lại để audit trail vẫn sống. Dùng khi disable không đủ — ví dụ test account, credential bị compromise cần bốc hẳn. |
